...
import { ProgramContext, ExperimentDeclarationContext, ExperimentHeaderContext, ExperimentBodyContext, SpaceDeclarationContext, SpaceHeaderContext, SpaceBodyContext, ControlBlockContext, SimpleTransitionContext, ConditionalTransitionContext, DataDefinitionContext, StrategyStatementContext, ParamDefinitionContext, TaskConfigurationContext, ParamAssignmentContext, WorkflowNameReadContext, SpaceNameReadContext, TaskNameReadContext, ParamValueContext, EnumFunctionContext, RangeFunctionContext } from '@extremexp/core/src/language/generated/ESPACEParser.js';
...
export class EspaceSymbolTableBuilder extends AbstractParseTreeVisitor<DocumentSymbolTable> implements ESPACEVisitor<DocumentSymbolTable> {
    ...
    visitExperimentHeader(ctx: ExperimentHeaderContext): DocumentSymbolTable {
        ...
        const experimentSymbol = this.addSymbol(ExperimentSymbol, experimentName, ctx, experimentName, this.document);
    ...
    visitSpaceHeader(ctx: SpaceHeaderContext): DocumentSymbolTable {
        ...
        if (!identifier || !workflowNameCtx) {
            this.addDiagnostic(ctx, 'Space name and workflow reference are required', DiagnosticSeverity.Error);
            return this.defaultResult();
        }
        ...
        const spaceSymbol = this.addSymbol(SpaceSymbol, spaceName, ctx, spaceName, workflowName, this.document);
        if (!spaceSymbol) return this.defaultResult();

        this.definedSpaces.add(spaceName);
        this.currentSpace = spaceSymbol;
        this.currentScope = spaceSymbol;
        return this.defaultResult();
    }
    ...
    visitControlBlock(ctx: ControlBlockContext): DocumentSymbolTable {
        if (!this.currentExperiment) {
            this.addDiagnostic(ctx, 'Control block must be defined within an experiment', DiagnosticSeverity.Error);
            return this.visitChildren(ctx);
        }
    ...
    visitConditionalTransition(ctx: ConditionalTransitionContext): DocumentSymbolTable {
        ...
        if (spaceReads.length !== 2) {
            this.addDiagnostic(header, 'Conditional transition must have exactly two spaces', DiagnosticSeverity.Error);
            return this.visitChildren(ctx);
        }
        ...
        if (!conditions || conditions.length === 0) {
            this.addDiagnostic(ctx, 'Conditional transition must have at least one condition', DiagnosticSeverity.Warning);
        }
    ...
    visitStrategyStatement(ctx: StrategyStatementContext): DocumentSymbolTable {
        if (!this.currentSpace) {
            this.addDiagnostic(ctx, 'Strategy can only be defined within a space', DiagnosticSeverity.Error);
            return this.visitChildren(ctx);
        }
    ...
    visitTaskConfiguration(ctx: TaskConfigurationContext): DocumentSymbolTable {
        if (!this.currentSpace) {
            this.addDiagnostic(ctx, 'Task configuration can only be defined within a space', DiagnosticSeverity.Error);
            return this.visitChildren(ctx);
        }
    ...
    private validateExperimentNameMatchesFileName(identifier: TerminalNode, experimentName: string): void {
    ...
    private validateTaskInWorkflow(ctx: ParserRuleContext, taskName: string): void {
        ...
        this.addDiagnostic(ctx, `Task validation for '${taskName}' in workflow '${this.currentSpace.workflowName}' not fully implemented`, DiagnosticSeverity.Information);
    }
    ...
    private extractRangeValue(rangeCtx: RangeFunctionContext): number[] {
        ...
        if (numbers.length !== 3) {
            this.addDiagnostic(rangeCtx, 'Range function must have exactly 3 numbers: min, max, step', DiagnosticSeverity.Error);
            return [];
        }

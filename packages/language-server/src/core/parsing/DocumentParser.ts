...
export class DocumentParser {
  ...
  public parseXxpDocument(textDocument: TextDocument, document: Document): void {
    ...
    try {
      ...
      this.checkForExtraContent(
        ...
        'Extra content found after workflow declaration.',
        document.diagnostics,
        textDocument
      );

      const folderSymbolTable = this.getOrCreateFolderSymbolTable(textDocument.uri);
      const symbolTableBuilder = new XxpSymbolTableBuilder(
  ...
  public parseEspaceDocument(textDocument: TextDocument, document: Document): void {
    ...
    try {
      ...
      this.checkForExtraContent(
        document.tokenStream,
        'Extra content found after experiment declaration.',
        document.diagnostics,
        textDocument
      );

      const folderSymbolTable = this.getOrCreateFolderSymbolTable(textDocument.uri);
      const symbolTableBuilder = new EspaceSymbolTableBuilder(
        this.documentManager,
        document,
        folderSymbolTable
      );

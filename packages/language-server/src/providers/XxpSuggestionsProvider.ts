...
import { DocumentSymbolTable } from '../language/symbolTable/DocumentSymbolTable';
...
export class XxpSuggestionsProvider extends Provider {
	...
	private static readonly ignoredTokens = new Set([XXPParser.NUMBER, XXPParser.COMMENT, XXPParser.STRING]);
	private static readonly preferredRules = new Set([XXPParser.RULE_workflowNameRead, XXPParser.RULE_dataNameRead, XXPParser.RULE_taskNameRead]);
	...
	private async onCompletion(params: CompletionParams): Promise<CompletionItem[] | null> {
		...
		await this.fixTokensSuggestionForChains(candidates.tokens, tokenPosition, document.tokenStream!, document.symbolTable!);
		await this.fixRulesSuggestionForChains(candidates.rules, tokenPosition, document.tokenStream!, document.symbolTable!);
		symbols.push(... await this.processRules(candidates.rules, tokenPosition, document.symbolTable!));
	...
	private async processRules(rules: Map<number, ICandidateRule>, position: TokenPosition, symbolTable: DocumentSymbolTable): Promise<CompletionItem[]> {
		const proposedSymbols: CompletionItem[] = [];

		if (rules.has(XXPParser.RULE_workflowNameRead)) {
			await this.suggestAndStoreSymbols(position, WorkflowSymbol, proposedSymbols, symbolTable, CompletionItemKind.Class);
		}
	...
	private async fixTokensSuggestionForChains(tokens: Map<number, TokenList>, position: TokenPosition, tokenStream: CommonTokenStream, symbolTable: DocumentSymbolTable): Promise<void> {
	...
	private async fixRulesSuggestionForChains(rules: Map<number, ICandidateRule>, position: TokenPosition, tokenStream: CommonTokenStream, symbolTable: DocumentSymbolTable): Promise<void> {
	...
	private async suggestAndStoreSymbols<T extends BaseSymbol>(position: TokenPosition, type: new (...args: any[]) => T, proposedSymbols: CompletionItem[], symbolTable: DocumentSymbolTable, kind: CompletionItemKind): Promise<void> {

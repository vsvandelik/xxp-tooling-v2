...
export class EspaceSuggestionsProvider extends Provider {
  ...
  private async processRules(
    ...
    if (rules.has(ESPACEParser.RULE_taskNameRead)) {
      ...
      await this.suggestTasksFromReferencedWorkflow(
        position,
        proposedSymbols,
        symbolTable,
        documentUri
  ...
  private async suggestAndStoreSymbols<T extends BaseSymbol>(
    ...
    (await symbolTable.getValidSymbolsAtPosition(position.parseTree, documentUri, type)).forEach(
      s => {
        proposedSymbols.push({
          label: s,
          kind,
  ...
  private async suggestTasksFromReferencedWorkflow(
    ...
    const spaceScopeSymbol = await symbolTable.getCurrentScopeSymbolByType(
      position.parseTree,
      documentUri,
      SpaceScopeSymbol
    );

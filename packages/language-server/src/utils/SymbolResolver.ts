...
export class SymbolResolver {
  ...
  static async resolveEspaceParameter(
    ...
    console.error(`[RESOLVER] resolveEspaceParameter: text="${text}"`);
    ...
    if (!experimentSymbol) {
      console.error(`[RESOLVER] resolveEspaceParameter: No experimentSymbol found`);
      return null;
    }

    console.error(`[RESOLVER] resolveEspaceParameter: Found experimentSymbol "${experimentSymbol.name}"`);

    // First try local resolution in experiment
    const localSymbol = await experimentSymbol.resolve(text, true);
    if (localSymbol) {
      console.error(`[RESOLVER] resolveEspaceParameter: Found locally: "${localSymbol.name}"`);
      return localSymbol;
    }

    console.error(`[RESOLVER] resolveEspaceParameter: Not found locally, checking workflow references`);

    // If not found locally, search in referenced workflows for parameters
    const spaces = await experimentSymbol.getSymbolsOfType(SpaceSymbol);
    console.error(`[RESOLVER] resolveEspaceParameter: Found ${spaces.length} spaces`);
    
    for (const space of spaces) {
      console.error(`[RESOLVER] resolveEspaceParameter: Checking space "${space.name}"`);
      
      if (space.workflowReference) {
        console.error(`[RESOLVER] resolveEspaceParameter: Space "${space.name}" has workflowReference "${space.workflowReference.name}"`);
        
        // Look for parameters in the referenced workflow
        const paramSymbol = await space.workflowReference.resolve(text, false);
        if (paramSymbol) {
          console.error(`[RESOLVER] resolveEspaceParameter: Found parameter in workflow "${space.workflowReference.name}": "${paramSymbol.name}" (type: ${paramSymbol.constructor.name})`);
          return paramSymbol;
        } else {
          console.error(`[RESOLVER] resolveEspaceParameter: Parameter "${text}" not found in workflow "${space.workflowReference.name}"`);
        }
      } else {
        console.error(`[RESOLVER] resolveEspaceParameter: Space "${space.name}" has no workflowReference`);
      }
    }

    console.error(`[RESOLVER] resolveEspaceParameter: Parameter "${text}" not found in any workflow references`);
    return null;
  }
}
...
import { AbstractParseTreeVisitor, CharStream, CommonTokenStream, Lexer, Parser, ParseTree, Token } from 'antlr4ng';
import { DocumentManager } from '../../core/managers/DocumentsManager';
import { Diagnostic, DiagnosticSeverity } from 'vscode-languageserver';
import { DiagnosticReportingErrorListener } from './DiagnosticReportingErrorListener';
import { Logger } from '../../utils/Logger';
import { DocumentSymbolTable } from '../symbolTable/DocumentSymbolTable';
import { XxpSpaceSymbolTableBuilder } from '../symbolTable/builders/EspaceSymbolTableBuilder';
import { XxpSymbolTableBuilder } from '../symbolTable/builders/XxpSymbolTableBuilder';
import { XxpDocument } from '../../core/documents/XxpDocument';
import { EspaceDocument } from '../../core/documents/EspaceDocument';
...
export class DocumentParser {
	...
	public parseDocument(textDocument: TextDocument, parsedDocument: Document, forcedFsParsing: boolean = false): void {
	...
	private parseDocumentHelper(
		...
		getSymbolTableBuilder: (documentsManager: DocumentManager, document: Document, symbolTable: DocumentSymbolTable) => AbstractParseTreeVisitor<DocumentSymbolTable>,
		...
		try {
			...
			parsedDocument.workflowSymbolTable = symbolTable.children.find(c => c instanceof WorkflowSymbol && c.document === parsedDocument) as WorkflowSymbol;
		} catch (error) {
	...
	private parseXxpDocument(textDocument: TextDocument, parsedDocument: Document, forcedFsParsing: boolean): void {
		this.parseDocumentHelper(
			...
			(documentsManager, document, symbolTable) => new XxpSymbolTableBuilder(documentsManager, document, symbolTable),
			'Extra content found after the end of the workflow declaration.',
			forcedFsParsing
		);
	}

	private parseEspaceDocument(textDocument: TextDocument, parsedDocument: Document, forcedFsParsing: boolean): void {
		this.parseDocumentHelper(
			textDocument,
			parsedDocument,
			...
			(documentsManager, document, symbolTable) => new XxpSpaceSymbolTableBuilder(documentsManager, document, symbolTable),
